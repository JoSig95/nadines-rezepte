...
9. Alle Rezeptkarten verwenden dieselbe gut lesbare Standardschriftgröße; die Schrift für umfangreiche Rezepte niemals verkleinern. Zunächst prüfen, ob das Rezept auf eine DIN-A5-Seite passt. Bei langen Rezepten Formulierungen kürzen und Abstände moderat anpassen; keine Zutaten oder Arbeitsschritte entfernen. Reicht eine Seite mit der Standardschrift nicht aus, das Rezept mit `cardPages: 2` auf zwei DIN-A5-Karten aufteilen: Titel, Bild und Zutaten auf Karte 1, Zubereitung und Tipp auf Karte 2. Bei zweispaltigen Zutatenlisten jede Zeile in einer expliziten Spalte rendern und prüfen, dass mehrzeilige Einträge nicht am Spaltenwechsel abgeschnitten werden. Auf jeder gedruckten Karte muss zwischen der letzten sichtbaren Inhaltszeile und dem unteren Blattrand mindestens 10 mm freier Rand bleiben. Bei einer zu langen Folgeseite zuerst mit `hideContinuationMeta: true` die drei Meta-Angaben unter der Überschrift ausblenden und nur Abstände moderat verkleinern; Schriftgrößen bleiben unverändert. Reicht der Platz danach weiterhin nicht, den Inhalt früher auf eine weitere Karte aufteilen. Vor Freigabe die letzte Zeile jeder Karte im tatsächlichen DIN-A5-Druckmaß prüfen.
...
11. Bei jeder Veröffentlichung eines neuen Rezepts den Versionsverlauf in `data/version-history.json` und `data/version-history.js` identisch aktualisieren. Die sichtbare Liste enthält immer genau die fünf zuletzt hinzugefügten Rezepte, absteigend nach Veröffentlichung, mit Version, Veröffentlichungsdatum, Rezeptnummer, Rezept-ID und Rezeptname. Ältere Einträge entfernen, sobald ein sechster Eintrag hinzukommt. Die Versionsnummer folgt dem Schema `1.<Rezeptnummer>.0`; `currentVersion` entspricht stets dem neuesten Eintrag. Den Versionsverlauf auf der Webseite und den Sprung zum jeweiligen Rezept prüfen.
12. JSON und JavaScript validieren und die Seite sowohl direkt per Doppelklick auf `index.html` als auch über einen lokalen Webserver prüfen.
