...
11. Bei jeder Veröffentlichung eines neuen Rezepts den Versionsverlauf in `data/version-history.json` und `data/version-history.js` identisch aktualisieren. Die sichtbare Liste enthält immer genau die zehn zuletzt hinzugefügten Rezepte, absteigend nach Veröffentlichung, mit Version, Veröffentlichungsdatum, Rezeptnummer, Rezept-ID und Rezeptname. Ältere Einträge entfernen, sobald ein elfter Eintrag hinzukommt. Die Versionsnummer folgt dem Schema `1.<Rezeptnummer>.0`; `currentVersion` entspricht stets dem neuesten Eintrag. Den Versionsverlauf auf der Webseite und den Sprung zum jeweiligen Rezept prüfen.
